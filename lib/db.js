...
const DEFAULT_DB_PATH = process.env.NODE_ENV === "production"
  ? "/data/checks.db"
  : "./checks.db";
const FALLBACK_DATA_PATH = "/data/checks.db";

let DB_PATH = process.env.DB_PATH || DEFAULT_DB_PATH;
if (!process.env.DB_PATH && fs.existsSync(FALLBACK_DATA_PATH)) {
  DB_PATH = FALLBACK_DATA_PATH;
  console.log(`ℹ️  Falling back to existing data path: ${DB_PATH}`);
}
        ...
        const runNextMigration = () => {
          ...
          db.run(stmt, (stmtErr) => {
            if (stmtErr) {
              ...
              const duplicateColumn = /duplicate column name/i.test(msg);
              const alreadyExists = /already exists/i.test(msg);

              if (duplicateColumn || alreadyExists) {
                console.warn(
                  `⚠️  Skipping already-applied migration statement for ${migration.name}: ${msg}`
                );
                runNextStatement();
                return;
              }

              console.error(`Migration failed [${migration.name}]:`, msg);
